...
def test_ekf():
    ...
    result = dyne.run_ekf(p.X0, p.P0, p.f, p.Q, p.n_epochs, p.measurements)
...
def test_ukf():
    ...
    for alpha in [1e-2, 1e-1, 1, 2]:
        result = dyne.run_ukf(p.X0, p.P0, p.f, p.Q, p.n_epochs, p.measurements,
                              alpha=alpha)
        en = (result.X - p.Xt) / np.diagonal(result.P, axis1=1, axis2=2) ** 0.5
        assert np.all(dyne.util.compute_rms(en) > 0.7)
        assert np.all(dyne.util.compute_rms(en) < 1.3)
...
def test_optimization():
    p = dyne.examples.generate_nonlinear_pendulum()
    result = dyne.run_optimization(p.X0, p.P0, p.f, p.Q, p.n_epochs, p.measurements)
...
def test_mhf():
    ...
    for window in [1, 3, 5]:
        result = dyne.run_mhf(p.X0, p.P0, p.f, p.Q, p.n_epochs, p.measurements, window)
        en = (result.X - p.Xt) / np.diagonal(result.P, axis1=1, axis2=2) ** 0.5
        assert np.all(dyne.util.compute_rms(en) > 0.7)
        assert np.all(dyne.util.compute_rms(en) < 1.3)
